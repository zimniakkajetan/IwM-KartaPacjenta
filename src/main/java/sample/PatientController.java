...
public class PatientController {
    ...
    private HashMap<Medication,ArrayList<Substance>> substances = new HashMap<>();
    ...
    @FXML
    VBox VBoxObservations,VBoxMedStatements,VBoxMedications;
    ...
    private void getPatientData() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                displayData();
                getMedications(medicationStatements);
                displayMedications();
    ...
    private void getMedications(ArrayList<MedicationStatement> medStatements){
        System.out.println("***MEDICATIONS***");
        for(MedicationStatement statement : medStatements){
            if (statement.getMedication() instanceof CodeableConceptDt) {
                CodeableConceptDt medicationCodeable = ((CodeableConceptDt) statement.getMedication());
                String code =medicationCodeable.getCodingFirstRep().getCode();
                Bundle results = client.search().forResource(Medication.class)
                ...
                for (final Bundle.Entry element : results.getEntry()) {
                    ...
                    if (resource instanceof Medication) {
                        ...
                        final Medication localMedication=medication;
                        if(medications.stream().filter(m->m.getId().getIdPart().equals(localMedication.getId().getIdPart())).count()==0) {
                        ...
                        if(substances.get(medication)!=null){
                            substances.get(medication).add((Substance)resource);
                        }else{
                            substances.put(medication,new ArrayList(Arrays.asList((Substance)resource)));
                        }
                    }
                }
            }
        }
        System.out.println("***MEDICATIONS END***");
    }

    private void displayMedications(){
        Platform.runLater(()->{
            for(int i=0;i<medications.size();i++){
                VBoxMedications.getChildren().add(createMedicationItem(medications.get(i),i));
            }
        });
    }
    ...
    private Node createMedicationStatementItem(String medName, String description, MedicationStatement mStatement, int id) {
        ...
        SVGGlyph icon = new SVGGlyph("M5,3C3.89,3 3,3.89 3,5V19C3,20.11 3.89,21 5,21H19C20.11,21 21,20.11 21,19V5C21,3.89 20.11,3 19,3H5M5,5H19V19H5V5M7,7V9H17V7H7M7,11V13H17V11H7M7,15V17H14V15H7Z");
        icon.setSize(25);
        icon.setFill(Paint.valueOf("white"));
        StackPane pane = new StackPane();
        pane.setPrefWidth(50);
    ...
    private Node createMedicationItem(Medication medication, int id) {
        HBox hbox = new HBox();
        hbox.setPadding(new Insets(10, 16, 10, 16));
        hbox.setSpacing(16);
        hbox.getStyleClass().add("HBoxObservation");
        ...
        vbox.setAlignment(Pos.CENTER_LEFT);
        Format formatter = new SimpleDateFormat("dd.MM.yyyy");
        vbox.getChildren().add(new Text(getDescriptionFromDiv(medication.getText().getDivAsString())));
    ...
    private void openMedicationDialog(Medication medication){
        JFXDialogLayout content = new JFXDialogLayout();
        content.setHeading(new Text("Medication details"));
        String medicationString = getDescriptionFromDiv(medication.getText().getDivAsString())+"\n\n";
        medicationString+="Ingredients:"+"\n";
        int i=0;
        for(Substance substance : substances.get(medication)){
            //((RatioDt)medication.getProduct().getIngredient().get(i).getAmount()).getDenominator().
            medicationString+="- "+substance.getCode().getCoding().get(0).getDisplay()+"\n";
            i++;
        }
        content.setBody(new Text(medicationString));
